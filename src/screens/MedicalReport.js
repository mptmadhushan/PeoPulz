...
export default function MedicalReport({ navigation, route }) {
  const dispatch = useDispatch();
  const userAuthToken = useSelector(state => state.auth.token);

  const [MedicalReport, setRowMaterial] = useState(initialData);
        ...
        <View style={styles.centerFlex}>
          {MedicalReport.image && (
            <View
              ...
              <Image
                source={{
                  uri: 'https://www.teahub.io/photos/full/42-422370_healthcare-medical-cover-page-design.jpg',
                }}
                resizeMode="contain"
                style={{
