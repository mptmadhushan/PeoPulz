...
export default function PossibleCauses({ navigation, route }) {
  const dispatch = useDispatch();
  const userAuthToken = useSelector(state => state.auth.token);

  const [PossibleCauses, setRowMaterial] = useState(initialData);
  ...
  const data = [
    {
      name: 'Aliquip aute commodo voluptate ex ut in ut nostrud.',
    },
    {
      name: 'Et exercitation pariatur qui Lorem.',
    },
    {
      name: 'Ullamco do esse incididunt',
    },
    {
      name: 'Ipsum nisi id et proident eiusmod sit dolor voluptate incididunt.',
    },
    {
      name: 'Enim non duis',
    },
  ...
  return (
    <SafeAreaView style={styles.container}>
      <ScrollView>
        ...
        >
          <Text style={styles.buttonTextStyle}>Temporary Solution</Text>
        </TouchableOpacity>
        <TouchableOpacity
          style={styles.buttonStyleCovi}
          activeOpacity={0.5}
          // onPress={() => navigation.navigate('causes')}
        >
          <Text style={styles.buttonTextStyle}>Doctor Channeling</Text>
        </TouchableOpacity>
      </ScrollView>
    </SafeAreaView>
  );
